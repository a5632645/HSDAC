...
static volatile uint8_t report_compelete_ = 0;
volatile uint8_t rx_has_data = 0;
...
__attribute__ ((aligned (4))) static uint8_t state_;
void HID_ReportOK (uint8_t state) {
    state_ = state;
    USBHSD->UEP1_TX_DMA = (uint32_t)&state_;
    USBHSD->UEP1_TX_LEN = 1;
    USBHSD->UEP1_TX_CTRL = (USBHSD->UEP1_TX_CTRL & ~USBHS_UEP_T_RES_MASK) | USBHS_UEP_T_RES_ACK;
    while (report_compelete_ == 0) {}
    report_compelete_ = 0;
}

uint8_t HID_HasData (void) {
    return rx_has_data;
}

void HID_NextData (void) {
    rx_has_data = 0;
    USBHSD->UEP1_RX_CTRL = (USBHSD->UEP1_RX_CTRL & ~USBHS_UEP_R_RES_MASK) | USBHS_UEP_R_RES_ACK;
}

uint8_t* HID_GetDataPtr (void) {
    return USBHS_EP1_Rx_Buf;
}

uint32_t HID_GetDataLen (void) {
    return rx_len;
}